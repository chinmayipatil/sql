...
(regno int foreign key references studentDetails(regno),
...
select cd.cname ,cr.cId,count(sd.regno) from studentDetails sd
join courseRegistration cr on sd.regno=cr.regno
join courseDetails cd on cd.cId=cr.cid
where start_date>='2018-01-02' and end_date<='2018-02-28'
group by cd.cname,cr.cId
...
-------------------------------------------------------------------
--12th query
select cd.cname from studentDetails st
join courseRegistration cr
on st.regno=cr.regno
